...
[Contribution Guide](contribution-guide.mdx)._
...
1. Identify which category file the term belongs in:
   ...
   - `/tools-techniques` - tools, software, methods
   - `workflows-methodologies.mdx` - development, documentation workflows for API projects
   - `frameworks-strategy.mdx` - conceptual models, strategic approaches for API documentation work
   - `writing-style.mdx` - language conventions, tone guidelines, rhetorical approaches
...
<ZoomImage 
  src="/api-docs-glossary/img/flowchart-style-guide-category-placement.png" 
  alt="Style Guide Category Placement flowchart showing how a contributor determines which category a glossary term belongs in based on what the term fundamentally describes, when readers need it, and its distinguishing characteristics"
/>
...
[API Docs Glossary](../introduction.mdx),
...
| --------- | ------ | ---------- | ----- |
| **Project/Product Names** | "docs" or "doc" | [ADG](../introduction.mdx), [Write the Docs](https://www.writethedocs.org/), [Doc Detective](https://doc-detective.com/) | Branding choice - both are valid as a naming decision, not a grammar rule |
...
such as [branch](../tools-techniques/development-essentials.mdx#branch)
→ [merge](../tools-techniques/development-essentials.mdx#merge) →
[commit](../tools-techniques/development-essentials.mdx#commit) →
[rebase](../tools-techniques/development-essentials.mdx#rebase)
...
such as [CLI](../tools-techniques/development-essentials.mdx#cli) →
...
section names, or [RFC numbers](../tools-techniques/documentation-specific.mdx#rfc)
...
| ------------ | ----------- | ---------- |
| Terms are frequently confused or used interchangeably | [REST vs *REST*ful](../api-types-architectures.mdx#rest-vs-restful) - often treated as synonyms despite technical distinction | Create a comparison entry that explains both the technical distinction and practical usage |
...
6. Update the `contribution-guide.mdx` by adding the new term category to any lists
