...
<ZoomImage 
  src="/api-docs-glossary/img/flowchart-ai-apis.png" 
  alt="Flowchart showing phases plan, draft with AI, human review, refine and verify, publish, maintain"
/>
...
**Definition**: structured documentation files that define how AI agents
behave within a project, including workflow rules, role specifications,
coordination patterns, task procedures, and acceptance criteria; commonly
written in Markdown

**Purpose**: enables tech writers to govern AI agent behavior using
docs skills they already possess — progressive disclosure, audience analysis,
IA, and editorial judgment — applied to an AI audience rather than a human one
...
**Related Terms**: [AI agent](#ai-agent), [information architecture](frameworks-strategy.mdx#information-architecture),
[Large Language Model](#large-language-model), [Markdown](./tools-techniques/documentation-specific.mdx#markdown),
[orchestration](#orchestration), [progressive disclosure](writing-style.mdx#progressive-disclosure), [prompt engineering](#prompt-engineering)

**Sources**:

- [arXiv: 2601.20404 - Lulla et al., "On the Impact of AGENTS.md Files on the Efficiency of AI Coding Agents"](https://arxiv.org/abs/2601.20404)
- [arXiv: 2602.11988 - Gloaguen et al., "Evaluating AGENTS.md: Are Repository-Level Context Files Helpful for Coding Agents?"](https://arxiv.org/abs/2602.11988)
- [Dachary Carey: Agent-Friendly Documentation Spec](https://agentdocsspec.com/)
...
- [Vercel Blog: "AGENTS.md outperforms skills in our agent evals" by Jude Gao](https://vercel.com/blog/agents-md-outperforms-skills-in-our-agent-evals)
...
**Skills are prose** — [Markdown](./tools-techniques/documentation-specific.mdx#markdown)
files with frontmatter — not compiled code; they have the same authoring, maintenance, and
quality requirements as any other docs, and the same failure modes when those requirements
aren't met. Skills generated by AI without human editorial oversight introduce information
decay: iterative [LLM](#large-language-model) processing degrades factual content measurably
across revision chains, producing outputs that read well but quietly diverge from ground truth.
Tech writers are well-positioned to co-own skills as a docs channel — reviewing for accuracy,
ensuring consistency with existing docs, and building evals or skill validators into
[CI pipelines](./tools-techniques/development-essentials.mdx#cicd-pipeline)
...
<ZoomImage 
  src="/api-docs-glossary/img/flowchart-ai-agent-workflow.png" 
  alt="Flowchart showing stages: adding skill to Claude Code, tech writer asking agent to test, AI agent breaking the task down, reads docs, converts steps, runs tests, reports results"
/>

**Related Terms**: [agent configuration](#agent-configuration), [AI](#ai), [API documentation testing](workflows-methodologies.mdx#api-documentation-testing),
[docs-as-tests](workflows-methodologies.mdx#docs-as-tests), [eval](#eval), [Large Language Model](#large-language-model), [Markdown](./tools-techniques/documentation-specific.mdx#markdown),
[MCP server](#mcp-server), [metaprogramming](#metaprogramming), [Natural Language Processing](#natural-language-processing), [orchestration](#orchestration), [prompt engineering](#prompt-engineering), [RAG](#rag)
...
- [passo.uno: "Skills are docs, and docs need tech writers" by Fabrizio Ferri Benedetti](https://passo.uno/skills-are-docs/)
...
**Related Terms**: [AI](#ai), [Clayton's Docs](writing-style.mdx#claytons-docs),
[docs theater](writing-style.mdx#docs-theater), [genAI](#genai), [GitBook](./tools-techniques/documentation-specific.mdx#gitbook),
[Large Language Model](#large-language-model), [Mintlify](./tools-techniques/documentation-specific.mdx#mintlify)
...
**Related Terms**: [AI](#ai),
[guerrilla usability testing](workflows-methodologies.mdx#guerrilla-usability-testing),
...
**Definition**: abbreviation for "evaluation"; automated test for an AI
system that measures whether the system's output meets defined success
criteria for a given input

**Purpose**: enables API docs teams to validate AI agent behavior before
deployment, catch regressions when models or prompts change, and define
measurable success criteria that replace guesswork; evals compound in
value over an agent's lifecycle — early evals force teams to specify what
"done" looks like, while later evals protect against quality drift
...
**Example**: a docs team using an AI agent to generate release notes builds
an eval suite with three grader types — a deterministic style guide check using Vale,
an LLM-as-judge rubric scoring accuracy and tone, and periodic human review to calibrate
the LLM grader; when the team upgrades to a new model, the eval suite runs automatically
and reveals that the new model over-summarizes breaking changes, giving the team a concrete
regression to fix before shipping

**Related Terms**: [AI agent](ai-and-apis.mdx#ai-agent), [deterministic testing](workflows-methodologies.mdx#deterministic-testing),
[docs-as-tests](workflows-methodologies.mdx#docs-as-tests), [probabilistic testing](workflows-methodologies.mdx#probabilistic-testing)
...
**Related Terms**: [content](writing-style.mdx#content),
...
[liquid content](#liquid-content), [modular content](writing-style.mdx#modular-content),
[ontology](frameworks-strategy.mdx#ontology), [RAG](#rag),
...
**Related Terms**: [agent configuration](#agent-configuration), [AI](#ai), [AI agent](#ai-agent), 
[genAI](#genai), [liquid content](#liquid-content), [metaprogramming](#metaprogramming),
[Natural Language Processing](#natural-language-processing), [orchestration](#orchestration),
...
<ZoomImage 
  src="/api-docs-glossary/img/flowchart-liquid-content-paths.png" 
  alt="Flowchart showing how AI adapts, personalizes, and transforms docs to different users - junior devs, senior devs, and users driving/commuting"
/>

**Related Terms**: [content](writing-style.mdx#content),
[kinetic content](writing-style.mdx#kinetic-content), [Large Language Model](#large-language-model),
[modular content](writing-style.mdx#modular-content), [real-time](api-types-architectures.mdx#real-time),
[structured content](writing-style.mdx#structured-content)
...
[authentication](./core-concepts/api-fundamentals.mdx#authentication),
[JSON](./core-concepts/api-fundamentals.mdx#json), [JSON-RPC](api-types-architectures.mdx#json-rpc),
[metaprogramming](#metaprogramming), [orchestration](#orchestration),
[REST API](./api-types-architectures.mdx#rest-api), [webhook API](./api-types-architectures.mdx#webhook-api)
...
**Definition**: "writing code that writes code" - programming technique where code
reads, generates, analyzes, or transforms other programs as data; API docs contexts
encompass writing LLM skills, automation scripts, agentic workflows, and prompt
instructions that generate or maintain docs

**Purpose**: enables tech writers to automate repetitive documentation tasks —
release note generation, table updates, endpoint validation — and build reusable
instructions that shape how LLMs produce, transform, and surface docs;
_shifts writer effort from execution to architecture and context curation_

**Historical Context**: has roots in 1970-1980s list-processing languages, particularly
Lisp, where the ability to treat code as data enabled early AI applications on
specialized Lisp machine hardware; the paradigm re-emerges in LLM tooling with a
significant twist — prompt engineering can be understood as
[pseudocode](https://en.wikipedia.org/wiki/Pseudocode) that gained execution capability;
like pseudocode, prompts describe intent in natural language rather than formal syntax,
but unlike pseudocode, they are interpreted and acted upon directly by a model; when
those instructions shape how a model generates other content, prompt engineering becomes
metaprogramming — _the distinction between building a system and explaining it collapses_
...
[Claude Skills](https://claude.com/skills), [Cursor Rules](https://cursor.com/docs/rules),
[Copilot Agent Skills](https://github.blog/changelog/2025-12-18-github-copilot-now-supports-agent-skills/),
...
| [prompt engineering](#prompt-engineering) | instructs a model how to reason, structure output, or behave | applied metaprogramming for LLMs — instructions that shape how a model generates other content; the distinction blurs when prompts become reusable, parameterized systems |
...
**Example**: a tech writer authors a Claude Skill that instructs an LLM to generate
consistently structured endpoint descriptions from OpenAPI spec changes on every
release — the skill itself is a metaprogram: a set of instructions that produces
documentation code rather than writing each entry manually

**Related Terms**: [AI agent](#ai-agent), [CI/CD pipeline](./tools-techniques/development-essentials.mdx#cicd-pipeline),
[docs-as-code](workflows-methodologies.mdx#docs-as-code), [LLM](#large-language-model), [MCP server](#mcp-server),
[OpenAPI Specification](./core-concepts/documentation-specific.mdx#openapi-specification),
[orchestration](#orchestration), [prompt engineering](#prompt-engineering)

**Sources**:

- [DEV Community: "A Practical Guide to Metaprogramming in Python" by Karishma Shukla](https://dev.to/karishmashukla/a-practical-guide-to-metaprogramming-in-python-691)
- [passo.uno: "New habits for tech writers in the age of LLMs" by Fabrizio Ferri Benedetti](https://passo.uno/new-habits-tech-writers-ai-age/)
...
**Definition**: coordination of multiple AI models, tools, agents,
or automated processes toward a larger goal; in documentation
contexts, the design and management of systems where LLMs, APIs,
and automated workflows operate together to produce, validate,
or maintain content

**Purpose**: enables tech writers to move from executing individual
tasks to architecting systems that execute on their behalf —
release note pipelines, multi-step agentic workflows, and
LLM-powered validation chains are all orchestration; the writer's
role shifts from author to system designer and context curator
...
| [metaprogramming](#metaprogramming) | instructions that shape how a model or program behaves | operates at the instruction level; orchestration operates at the system level — coordinating *which* models, tools, and agents run, in what order, and with what context; a Claude Skill is metaprogramming, the pipeline that invokes it is orchestration |

**Example**: a tech writer designs an agentic CI/CD pipeline that
triggers on every API release — an LLM reads the OpenAPI diff,
a second agent drafts affected reference entries, a validation
step runs docs-as-tests against the new endpoints, and a final
agent opens a pull request with the changes; the writer authored
the system, not the content

**Related Terms**: [agent configuration](#agent-configuration), [AI agent](#ai-agent),
[docs-as-tests](workflows-methodologies.mdx#docs-as-tests), [Large Language Model](#large-language-model),
[metaprogramming](#metaprogramming), [MCP server](#mcp-server), [prompt engineering](#prompt-engineering)

**Sources**:

- [LangChain Docs: "LangChain overview"](https://python.langchain.com/docs/introduction/)
- [passo.uno: "New habits for tech writers in the age of LLMs" by Fabrizio Ferri Benedetti](https://passo.uno/new-habits-tech-writers-ai-age/)
...
**Related Terms**: [agent configuration](#agent-configuration), [AI](#ai),
[AI agent](#ai-agent), [genAI](#genai), [Large Language Model](#large-language-model),
[metaprogramming](#metaprogramming), [orchestration](#orchestration)
...
**RAG vs GraphRAG**: GraphRAG is an implementation approach that uses
knowledge graph structures to improve retrieval quality, but the fundamental
concept of grounding AI responses in source material remains the same
...
[Fin](./tools-techniques/documentation-specific.mdx#fin),
[Inkeep](./tools-techniques/documentation-specific.mdx#inkeep),
[Kapa.ai](./tools-techniques/documentation-specific.mdx#kapaai),
