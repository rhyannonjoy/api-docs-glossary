...
**Definition**: open source web framework that uses
[islands architecture](https://www.patterns.dev/vanilla/islands-architecture/)
to deliver fast, content-focused websites by rendering pages to static HTML and
selectively hydrating interactive components with JavaScript only where needed

**Purpose**: enables API docs teams to build performant docs sites with
minimal JavaScript overhead; supports multiple UI frameworks
[React](https://react.dev/), [Vue](https://vuejs.org/), [Svelte](https://svelte.dev/)
within the same project, allowing teams to use familiar tools while maintaining
excellent performance; [Starlight theme](https://starlight.astro.build/resources/themes/)
provides docs-specific features like automatic navigation, search, and
internationalization out of the box

**Example**: A technical writing team builds a developer portal using Astro with Starlight;

```markdown
1. Write docs in Markdown with frontmatter for metadata
2. Creates interactive API examples using React components - code playgrounds, response visualizers
3. Adds Vue-based configuration builders for complex endpoint customization
4. Astro renders everything to static HTML, only loading framework code for interactive islands
5. Deploys to a CDN with sub-second page loads despite rich interactivity
```

**Related Terms**: [docs-as-code](../workflows-methodologies.mdx#docs-as-code), [Docusaurus](#docusaurus),
[Markdown](#markdown), [MDX](#mdx), [MkDocs](#mkdocs), [static site generator](#static-site-generator)

**Sources**:

- [Astro Homepage: "The web framework for content-driven websites"](https://astro.build/)
- [Astro, Starlight: "Make your docs shine with Starlight"](https://starlight.astro.build/)

---

## CCMS

**Definition**: acronym for _component content management system_; platform that manages
documentation at the component level with individual topics, procedures, parameters,
warnings, or assets, rather than at the document level; each component is stored once and
reused by reference across multiple documents and outputs; commonly built on XML-based
structured authoring standards, most often
[DITA](https://en.wikipedia.org/wiki/Darwin_Information_Typing_Architecture) —
_Darwin Information Typing Architecture_ — or [DocBook](https://en.wikipedia.org/wiki/DocBook)

**Purpose**: enables API docs teams managing large, multi-product,
or multi-audience content sets to maintain a single source of truth for
shared components, as in parameter descriptions, authentication procedures,
error code tables, and publish them across reference docs, getting started
guides, SDK documentation, and localized outputs simultaneously; when a
shared component changes, the update propagates automatically to every
document that references it, eliminating the copy-paste drift that
degrades accuracy across large documentation sets; supports structured
authoring workflows, granular version control at the component level, and
conditional publishing for audience- or product-specific output variants

### CCMS vs CMS

| **Aspect** | **CCMS** | **CMS** |
| -------- | ----- | ----- |
| **Content Unit** | Individual Components: topics, paragraphs, assets | Full pages or documents |
| **Reuse Model** | Reference-based: one source, many outputs | Copy-based: content duplicated across pages |
| **Structure** | Requires structured authoring: DITA, DocBook XML | Supports unstructured or lightly structured content |
| **Version Tracking** | Per-component lifecycle: owner, version, approval tracked individually | Per-page, per-document versioning |
| **Publishing** | Multi-channel output, single component set: print, web, PDF, e-reader | Primarily web publishing |
| **Primary Users** | Technical writers managing complex, multi-product docs | Content creators, marketers, web managers |
| **API Docs Use Case** | Managing shared parameter definitions, procedures, error references across many docs | Managing endpoint reference pages, tutorials, guides as discrete pages |
| **Tools** | [Heretto](https://www.heretto.com/), [MadCap IXIA CCMS](https://www.madcapsoftware.com/), [Componize](https://www.componize.com/), [XDocs](https://xdocs.net/) | [Contentful](https://www.contentful.com/), [Strapi](https://strapi.io/), [WordPress](https://wordpress.com/) |

**Example**: an API doc team maintains a shared `auth-flow` component describing
the OAuth 2.0 authentication flow; the component is referenced in the getting
started guide, the API reference, the SDK docs for three languages, and the
localized Japanese output. When the auth flow changes, a writer updates the single
component and the CCMS automatically propagates the update to all seven outputs;
the same team uses a CCMS to conditionally publish product-tier variants, filtering
components by metadata so Enterprise and Developer plan docs render from the same
component library with different content visible per audience

**Related Terms**: [CMS](#cms), [content](../writing-style.mdx#content), [DAM](#dam),
[docs-as-code](../workflows-methodologies.mdx#docs-as-code), [information architecture](../frameworks-strategy.mdx#information-architecture),
[knowledge management](../frameworks-strategy.mdx#knowledge-management), [metadata](#metadata), [partials](#partials),
[structured content](../writing-style.mdx#structured-content), [taxonomy](../frameworks-strategy.mdx#taxonomy), [version control](development-essentials.mdx#version-control)

**Sources**:

- [Heretto Intellectual Property, Content Ops: "What Is a Component Content Management System (CCMS)?" by Ren Taylor](https://www.heretto.com/blog/component-content-management-system)
- [Wikipedia: "Component content management system"](https://en.wikipedia.org/wiki/Component_content_management_system)
...
| **Tools** | [Contentful](https://www.contentful.com/), [Strapi](https://strapi.io/), [WordPress](https://wordpress.com/), [Docusaurus](https://docusaurus.io/) | [Bynder](https://www.bynder.com/en/), [Adobe Experience Manager](https://business.adobe.com/products/experience-manager/adobe-experience-manager.html), [Cloudinary](https://cloudinary.com/) |
...
**Related Terms**: [CCMS](#ccms), [content](../writing-style.mdx#content), [DAM](#dam), [docs-as-code](../workflows-methodologies.mdx#docs-as-code),
[Git](../tools-techniques/development-essentials.mdx#git), [GUI](../tools-techniques/development-essentials.mdx#gui),
[knowledge management](../frameworks-strategy.mdx#knowledge-management), [metadata](#metadata), [SDK](../core-concepts/api-fundamentals.mdx#sdk),
[structured content](../writing-style.mdx#structured-content), [version control](development-essentials.mdx#version-control)
...
**Related Terms**: [CCMS](#ccms), [content](../writing-style.mdx#content), [CMS](#cms),
[docs-as-ecosystem](../frameworks-strategy.mdx#docs-as-ecosystem), [knowledge management](../frameworks-strategy.mdx#knowledge-management),
...
**Related Terms**: [API playground](#api-playground), [API sandbox](#api-sandbox), [docs-as-code](../workflows-methodologies.mdx#docs-as-code),
[DX](../frameworks-strategy.mdx#dx), [getting started](../core-concepts/documentation-specific.mdx#getting-started), [GitBook](#gitbook),
[Mintlify](#mintlify), [OpenAPI Specification](../core-concepts/documentation-specific.mdx#openapi-specification),
[reference](../core-concepts/documentation-specific.mdx#reference), [SDK](../core-concepts/api-fundamentals.mdx#sdk),
[Tome](#tome), [tutorial](../core-concepts/documentation-specific.mdx#tutorial)
...
**Related Terms**: [Astro](#astro), [docs-as-code](../workflows-methodologies.mdx#docs-as-code), [Fumadocs](#fumadocs),
[GitHub Pages](#github-pages), [Markdown](#markdown), [MDX](#mdx), [MkDocs](#mkdocs), [Nextra](#nextra),
[OpenAPI Specification](../core-concepts/documentation-specific.mdx#openapi-specification), [static site generator](#static-site-generator)
...
**Definition**: open source React.js docs framework built by Fuma Nama;
generates docs sites with MDX support, built-in search, OpenAPI playground, and
composable UI components; works with [Next.js](https://nextjs.org/),
[TanStack Start](https://tanstack.com/start/latest), [React Router](https://reactrouter.com/),
and [Waku](https://waku.gg/)

**Purpose**: gives engineering-leaning docs teams a headless, composable alternative
to opinionated platforms like [Docusaurus](https://docusaurus.io/); separates
content - `fumadocs-mdx`, core logic - `fumadocs-core`, and UI - `fumadocs-ui`, into distinct
layers so teams can adopt the pieces they need without committing to the full framework;
`fumadocs-openapi` extends the framework to render OpenAPI specifications as interactive
API reference docs; supports MDX content with
[TypeScript Twoslash](https://www.typescriptlang.org/dev/twoslash/) for inline type
information in code blocks, making it well-suited for documenting SDKs and typed APIs

### Fumadocs vs Docusaurus

| **Aspect** | **Fumadocs** | **Docusaurus** |
|---|---|---|
| **Framework Dependency** | React.js framework-agnostic — Next.js, TanStack Start, Waku | React-based SSG built and maintained by Meta |
| **Architecture** | Headless, composable — use content, core, and UI layers independently | Opinionated, batteries-included |
| **Setup Approach** | Compose from packages; requires React/Next.js familiarity | Preconfigured; lower barrier for non-engineers |
| **Customization** | High — swap UI layer entirely, integrate any CMS | Moderate — theme swizzling, plugin system |
| **OpenAPI Support** | `fumadocs-openapi` plugin | `docusaurus-plugin-openapi-docs` by Palo Alto Networks |
| **Best For** | Engineering teams with React/Next.js workflows | Broader teams including non-engineer contributors |

**Example**: a developer tools company building on Next.js initializes a Fumadocs
project with `pnpm create fumadocs-app`, adds `fumadocs-openapi` to render their
OpenAPI spec as an interactive reference, and extends the UI layer with custom
components without modifying the core content pipeline

**Related Terms**: [docs-as-code](../workflows-methodologies.mdx#docs-as-code), [Docusaurus](#docusaurus), [MDX](#mdx),
[MkDocs](#mkdocs), [OpenAPI Specification](../core-concepts/documentation-specific.mdx#openapi-specification),
[static site generator](documentation-specific.mdx#static-site-generator)

**Sources**:

- [Fumadocs Homepage: "Build excellent documentations, your style"](https://www.fumadocs.dev/)
- [GitHub Repository: fuma-nama/fumadocs](https://github.com/fuma-nama/fumadocs)

---

## GitBook

**Definition**: cloud-based docs platform that combines a
[WYSIWYG](https://en.wikipedia.org/wiki/WYSIWYG) editor with GitHub and GitLab sync,
publishing polished docs sites for public developer portals, API references, help
centers, and internal knowledge bases

**Purpose**: bridges the gap between technical and non-technical contributors by
supporting both Git-based workflows and a visual editor on the same platform;
engineers commit docs changes through GitHub or GitLab sync while writers and
product managers use the WYSIWYG interface — both workflows publish to the same
destination; GitBook Agent proactively surfaces improvement suggestions to keep
docs aligned with product changes; the built-in AI Assistant answers reader questions
using the published docs content; positions itself explicitly as the buy-vs-build
alternative for teams that have outgrown custom docs infrastructure

### GitBook vs Mintlify

| **Aspect** | **GitBook** | **Mintlify** |
|---|---|---|
| **Primary Audience** | Mixed teams — technical and non-technical contributors | Devs and API-first teams |
| **Editor Experience** | WYSIWYG + Git sync — contributors choose their preferred workflow | Docs-as-code first — MDX and OpenAPI in Git |
| **OpenAPI Rendering** | Supported | Core feature — auto-generates playground from spec |
| **AI Features** | GitBook Agent - maintenance suggestions + AI Assistant - reader Q&A | AI-assisted writing |
| **Hosting** | Managed cloud | Managed cloud |
| **Best For** | Teams with mixed tech audiences - help centers, internal knowledge bases | Teams publishing interactive API references |

**Example**: a small docs team uses GitBook to publish an external developer portal —
engineers push API reference updates through GitHub sync, while a customer success
writer updates the help center through the WYSIWYG editor; both workflows publish to
the same GitBook site without requiring writers to learn Git

**Related Terms**: [AI-assisted documentation](../ai-and-apis.mdx#ai-assisted-documentation), [developer portal](#developer-portal),
[docs-as-code](../workflows-methodologies.mdx#docs-as-code), [DX](../frameworks-strategy.mdx#dx), [Git](development-essentials.mdx#git),
[knowledge management](../frameworks-strategy.mdx#knowledge-management), [Mintlify](#mintlify),
[OpenAPI Specification](../core-concepts/documentation-specific.mdx#openapi-specification)

**Source**: [GitBook Inc.: "Buy vs Build: Don't build a docs platform. Just create great docs."](https://www.gitbook.com/buy-vs-build)
...
**Related Terms**: [agent configuration](../ai-and-apis.mdx#agent-configuration), [Astro](#astro),
...
[MkDocs](#mkdocs), [Nextra](#nextra), [partials](#partials), [rST](#rst), [Tome](#tome), [Vale](#vale)
...
**Related Terms**: [agent configuration](../ai-and-apis.mdx#agent-configuration), [Astro](#astro), [Git](development-essentials.mdx#git),
[Git Bash](development-essentials.mdx#git-bash), [GitHub](development-essentials.mdx#github), [MDX](#mdx),
[Mermaid](diagramming-visualization.mdx#mermaid), [MkDocs](#mkdocs), [Nextra](#nextra), [partials](#partials), [rST](#rst), [Tome](#tome), [Vale](#vale)
...
**Related Terms**: [Astro](#astro), [docs-as-code](../workflows-methodologies.mdx#docs-as-code), [Docusaurus](#docusaurus), [Fumadocs](#fumadocs),
[GUI](development-essentials.mdx#gui), [knowledge graph](../ai-and-apis.mdx#knowledge-graph), [Markdown](#markdown), [Mermaid](diagramming-visualization.mdx#mermaid),
[Mintlify](#mintlify), [Nextra](#nextra), [partials](#partials), [rST](#rst), [Tome](#tome), [UI](development-essentials.mdx#ui)
...
**Related Terms**: [CCMS](#ccms), [CMS](#cms), [content](../writing-style.mdx#content),
[DAM](#dam), [information architecture](../frameworks-strategy.mdx#information-architecture),
[knowledge graph](../ai-and-apis.mdx#knowledge-graph), [knowledge management](../frameworks-strategy.mdx#knowledge-management),
...
**Related Terms**: [AI-assisted documentation](../ai-and-apis.mdx#ai-assisted-documentation), [API playground](#api-playground),
[developer portal](#developer-portal), [docs-as-code](../workflows-methodologies.mdx#docs-as-code), [Git](development-essentials.mdx#git),
[GitBook](#gitbook), [MDX](#mdx), [OpenAPI Specification](../core-concepts/documentation-specific.mdx#openapi-specification),
[Swagger](#swagger), [Tome](#tome), [version control](development-essentials.mdx#version-control)
...
**Related Terms**: [Astro](#astro), [docs-as-code](../workflows-methodologies.mdx#docs-as-code), [Docusaurus](#docusaurus), [Fumadocs](#fumadocs),
[GitHub Pages](#github-pages), [Markdown](#markdown), [rST](#rst), [static site generator](#static-site-generator)
...
**Definition**: [Next.js](https://nextjs.org/)-based documentation framework that combines
[React](https://react.dev/), MDX, and static site generation to create interactive docs
sites with minimal configuration

**Purpose**: streamlines API docs creation for teams already using Next.js or React by
providing docs-specific features like automatic sidebar generation, full-text search,
and syntax highlighting; integrates seamlessly with Next.js deployment workflows and
supports dynamic content generation alongside static pages; commonly used for developer
portals, API references, and technical docs requiring custom interactive components

**Example**: SaaS company documents their API using Nextra; the shared tech stack
enables docs to stay synchronized with product releases and reuse existing components -

```markdown
1. Engineering team already deploys their product on Vercel using Next.js
2. Tech writers create docs site with Nextra in the same monorepo
3. Writers use MDX to embed custom React components - API key generators, live request builders
4. Nextra generates static pages with automatic navigation and search
5. Deploys alongside the product with shared authentication and styling
```

**Related Terms**: [docs-as-code](../workflows-methodologies.mdx#docs-as-code), [Docusaurus](#docusaurus),
[Markdown](#markdown), [MDX](#mdx), [monorepo](development-essentials.mdx#monorepo), [static site generator](#static-site-generator)

**Source**: [Vercel, The Nextra Project: "Make beautiful websites with Next.js & MDX](https://nextra.site/)
...
**Related Terms**: [CCMS](#ccms), [docs-as-code](../workflows-methodologies.mdx#docs-as-code),
[Docusaurus](#docusaurus), [Markdown](#markdown), [MDX](#mdx), [MkDocs](#mkdocs), [monorepo](development-essentials.mdx#monorepo),
[static site generator](#static-site-generator), [white-labeling](../frameworks-strategy.mdx#white-labeling)
...
**Definition**: acronym for _reStructuredText_; markup language designed for tech docs;
default format for Python docs tooling

**Purpose**: provides semantic markup capabilities beyond Markdown, enabling complex
docs structures with cross-references, directives, and extensibility; powers Python API
docs through [Sphinx](https://www.sphinx-doc.org/en/master/index.html) and
[Read the Docs](https://about.readthedocs.com/); supports tech writing needs like automatic
API reference generation, table of contents, and index creation

### reStructuredText vs Markdown

| **Aspect** | **reStructuredText** | **Markdown** |
|---|---|---|
| **Complexity** | More features, steeper learning curve | Straightforward syntax, faster onboarding |
| **Extensibility** | Directive system supports custom extensions | Limited extensibility without custom processors |
| **Semantic Markup** | Built-in support for admonitions, citations, cross-references | Requires extensions or HTML |
| **Python Ecosystem** | Standard for Python docs | Requires additional tooling |
| **Cross-References** | Native automatic linking between docs sections | Manual or plugin-dependent |
| **Adoption** | Dominant in Python projects | Dominant across many languages and platforms |

**Example**: a developer documents a Python REST API client library that enables consistent
formatting between inline code docs and standalone guide pages -

```markdown
1. Writes docstring in Python code using rST formatting
2. Creates `.rst` files for guides, tutorials, and architecture docs
3. Uses Sphinx to generate HTML docs automatically from docstring and `.rst` files
4. Deploys to Read the Docs for versioned, searchable API docs
```

**Related Terms**: [docs-as-code](../workflows-methodologies.mdx#docs-as-code), [Markdown](#markdown), [MDX](#mdx),
[MkDocs](#mkdocs), [static site generator](#static-site-generator)

**Sources**:

- [Docutils: "reStructuredText: Markup Syntax and Parser Component of Docutils"](https://docutils.sourceforge.io/rst.html)
- [Sphinx Documentation: "reStructuredText Primer"](https://www.sphinx-doc.org/en/master/usage/restructuredtext/basics.html)
...
**Related Terms**: [Astro](#astro), [CI/CD pipeline](development-essentials.mdx#cicd-pipeline), [docs-as-code](../workflows-methodologies.mdx#docs-as-code),
[Docusaurus](#docusaurus), [Fumadocs](#fumadocs), [GitHub Pages](#github-pages), [Markdown](#markdown), [MkDocs](#mkdocs),
[Nextra](#nextra), [rST](#rst), [Tome](#tome)
...
**Definition**: open source docs platform that generates docs sites from
Markdown and MDX files with built-in full-text search, API reference support,
and AI chat; available as a self-hosted deployment or through Tome Cloud

**Purpose**: provides a [Mintlify](https://www.mintlify.com/)-alternative docs
publishing workflow at no licensing cost for teams that want managed-platform
aesthetics without the SaaS pricing; self-hosted deployment gives teams full
control over infrastructure and data; Tome Cloud offers a hosted path for teams
that want managed deployment without building custom infrastructure

**Maturity**: Tome is an early-stage project;
[the GitHub repository](https://github.com/vxcozy/tome) has limited adoption
signals compared to Mintlify, [GitBook](https://www.gitbook.com/),
[Docusaurus](https://docusaurus.io/), and [MkDocs](https://www.mkdocs.org/) as
of early 2026; evaluate project activity before adopting for production use

**Example**: a startup with privacy requirements self-hosts Tome on their own
infrastructure to publish an API reference and developer portal, avoiding SaaS
licensing costs and keeping docs traffic off third-party servers; teams with
fewer infrastructure constraints deploy to Tome Cloud using the same Markdown
and MDX source files

**Related Terms**: [developer portal](#developer-portal), [docs-as-code](../workflows-methodologies.mdx#docs-as-code),
[Markdown](#markdown), [MDX](#mdx), [Mintlify](#mintlify), [OpenAPI Specification](../core-concepts/documentation-specific.mdx#openapi-specification),
[static site generator](documentation-specific.mdx#static-site-generator)

**Sources**:

- [Tome Homepage: "Beautiful docs, zero friction."](https://www.tome.center/)
- [GitHub Repository: vxcozy/tome](https://github.com/vxcozy/tome)
